...
#$BashBootstrapPath = Resolve-Path -Path "$PSScriptRoot\bootstrap.sh"
#& $Executable run 'bash "$("$(command -v wslpath)" -u '`'$BashBootstrapPath`'')" && exec $(command -v zsh) -l -c "exit"'
Write-Color -Text '✔️' -Color Green
